...
scripts/run_xai.py                optional post-hoc Grad-CAM and occlusion
...
```bash
python scripts/run_xai.py --config configs/final.yaml --max-images-per-class 100
python scripts/make_report_assets.py --config configs/final.yaml
```

`scripts/run_xai.py` computes saved XAI arrays and CSV statistics. Report tables and PNG figures are generated by `scripts/make_report_assets.py`.
